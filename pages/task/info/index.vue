            ...
            <template v-else>
              ...
              <template v-if="info.state == 1">
                <template v-if="info.is_up == 0">
                  <view class="padding-xs">
                    <view class="cu-tag" :class="'line-green'">已下架</view>
                  </view>
                </template>
    ...
    <view class="footer">
      <template v-if="info.is_owner == 1">
        ...
        <template v-if="info.task_state == 1">
          <button
            class="border-radius"
            size="large"
            @tap.stop="`/pages/amount/deposit?task_order=${task_order}&&price=${price}`"
          >待支付</button>
        </template>
        <template v-else>
          <!-- 支付完成进入待审核状态 -->
          <template v-if="info.state == 0">
            <button class="border-radius disabled" size="large">待审核</button>
          </template>
          <!-- 审核通过 -->
          <template v-if="info.state == 1">
            <!--  任务状态为0 招募中 -->
            <template v-if="info.task_state == 0">
              <template v-if="info.is_up == 0">
                <button class="border-radius" size="large" @tap="save1(info)">上架</button>
              </template>
              <template v-if="info.is_up == 0">
                <view class="box">
              ...
              <template v-if="info.is_up == 1">
                <button class="border-radius" size="large" @tap="save2(info)">下架</button>
              </template>
            </template>
            <!--  任务状态为2 进行中-->
            <template v-if="info.task_state == 2">
              <button class="border-radius" size="large" disabled>任务进行中</button>
              <view class="box">
                <span class="left"></span>
            ...
            <template v-if="info.task_state == 3">
              <button class="border-radius disabled" size="large">任务终止</button>
            </template>
            <template v-if="info.task_state == 4">
              <button class="border-radius disabled" size="large">任务完成</button>
            </template>
            <!-- 任务状态为5 完成待确认 -->
            <template v-if="info.task_state == 5">
              <button class="border-radius" size="large" @tap="confirm2(info)">确认完成</button>
            </template>
          </template>
          <!-- 审核被驳回 -->
          <template v-if="info.state == 2">
            <button
              class="border-radius"
              size="large"
              @tap="go(`/pages/task/edit?id=${info.id}`)"
            >重新编辑</button>
          </template>
        </template>
      </template>
      ...
      <template v-if="info.is_owner == 0">
        ...
        <template v-if="info.state == 1">
          ...
          <template v-if="info.task_state == 0">
            <template v-if="info.is_up == 0">
              <button class="border-radius disabled" size="large">任务已下架</button>
            </template>
            <template v-if="info.is_up == 1">
              <template v-if="info.is_join == 0">
                <button
                  class="border-radius"
                  size="large"
                  @tap="go(`/pages/enroll/index?id=${info.id}`)"
                >参与项目</button>
              </template>
              <template v-if="info.is_join == 1">
                <button class="border-radius disabled" size="large">已申请</button>
              </template>
            </template>
          </template>
          <template v-if="info.task_state == 2">
            <template v-if="info.is_join == 0">
              <button class="border-radius disabled" size="large">您已错过此次任务</button>
            </template>
            <template v-if="info.is_join == 1">
              <button class="border-radius disabled" size="large">您的申请没有通过</button>
            </template>
            <template v-if="info.is_join == 2">
              <button class="border-radius" size="large" @tap="confirm1(info)">确认完成</button>
            </template>
          </template>
          <template v-if="info.task_state == 3">
            <button class="border-radius disabled" size="large">任务中止</button>
          </template>
          <template v-if="info.task_state == 4">
            <button class="border-radius disabled" size="large">任务已完成</button>
          </template>
          <template v-if="info.task_state == 5">
            <button class="border-radius disabled" size="large">已完成待验收</button>
          </template>
        </template>
      </template>
    </view>
