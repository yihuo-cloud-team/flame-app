<template>
  <view id="add">
    ...
    <view class="tupian">
      ...
      <image mode="aspectFill" class="task-img" @click="upload" :src="$getUrl(form.img)" />
      <view class="default-text">
        <text @click="change">使用默认图片</text>
      </view>
    </view>
    ...
    <view class="price">
      ...
      <view class="price-box" v-if="priceList">
        <view class="price-card" @click="price(todo)" v-for="(todo,i) in priceList" :key="i">
          <view>{{todo}}</view>
        </view>
    ...
    <view class="address">
      <view class="title">任务周期(天)</view>
      <view class="input">
        <input maxlength="30" type="number" v-model.number="form.cycle" placeholder="请输入数字" />
      </view>
    </view>
    <view class="info">
    ...
    <view class="region">
      ...
      <view class="input" v-if="form.p==''" @click="show = !show">省市区选择</view>
      <view
        class="input"
        v-if="form.p!=''"
        @click="show = !show"
      >{{form.p | areaToStr}}{{form.c | areaToStr}}{{form.a | areaToStr}}</view>
    </view>
    <view class="address">
    ...
    <view class="btn" @click="submit">下一步：托管资金</view>
